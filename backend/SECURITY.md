...
- **Privilege model (2026-09-10):** `gsiv-platform.service` runs as the unprivileged `ubuntu` user, so every host mutation in the provisioner now goes through `sudo` — `sudo systemctl daemon-reload|enable --now|restart|stop|disable`, `sudo caddy validate|reload`, `sudo rm -f -- <drop-ins>`, and drop-in content staged in `tmpdir()` then `sudo install [-d] -m 644|755` into `/etc/systemd/system`. Still no shell strings: `execFile` with fixed argv arrays, and the only request-influenced value that can reach a privileged command is the `validateCharName`-checked character name embedded in unit/path names. `sudo` is NOPASSWD for that service account on this host (systemd control already required it). Caddyfile/backend `.env` keep the plain fs layer (service-owned) so their ownership/mode is never changed, and no privileged write target exists outside the injected `VELLUM_SYSTEMD_DIR`/`VELLUM_CADDYFILE`/`VELLUM_ENV_FILE` paths. Unit tests assert the exact privileged argv, including that drop-ins are installed (never plain-written) and removed via `sudo rm -f --` on rollback.
