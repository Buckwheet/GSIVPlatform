...
## 7. Session handoff — 2026-09-10 → next session (start here)

**Session log — 2026-09-10 (Aeton was on the TEST instance + auto-provision was dead; PRs #59/#60):**
The dashboard showed `Aeton` online while he was absent from the live game. Three separate causes, fixed in order:

1. **Wrong instance, two characters with the same name.** `entry.yaml` held two `Aeton` records: `KAISER999` (first match for `lich.rbw --login Aeton`, recorded as `game_code: GST` = GemStone IV **Prime Test**) and a stale `JEMLEY/Aeton` (`GS3`). SGE character lists showed `KAISER999` owns `Aeton` on **both GS3 and GST**, while `JEMLEY` has no Aeton at all (not on GS3, not on GST) — so the "live" record could never authenticate (`Authentication fatally failed: Error(CHARACTER_NOT_FOUND)`) and every launch silently went to the test instance (log dir `logs/GST-Aeton/`; `logs/GSIV-Aeton/` untouched since 2026-06-15). Fixed by correcting KAISER999/Aeton to `game_code: GS3` + `game_name: GemStone IV` (backup `data/entry.yaml.bak-2026-09-10-aeton-live-pin`) and deleting the phantom `JEMLEY/Aeton` block (backup `data/entry.yaml.bak-2026-09-10-jemley-aeton-phantom`). Aeton now logs in as `GSIV:Aeton` (live) and the dashboard shows one Aeton row instead of two.
2. **The on-launch auto-provisioner (PR #56) had never worked in production.** `gsiv-platform.service` runs as `ubuntu`, but `core/stream-provision.ts` wrote `/etc/systemd/system` drop-ins with a plain `writeFileSync`/mkdirSync and called `systemctl`/`caddy` unprivileged → `POST /launch/Aeton` returned `HTTP 500 {"error":"provisioning failed and was rolled back: EACCES: permission denied, mkdir '/etc/systemd/system/gs4sd-lich@Aeton.service.d'"}`. Vaikar's stream (2026-08-17) had been the manual §VellumFE recipe. **PR #59**: every privileged step now goes through sudo — `privExec` (`sudo <cmd>` argv arrays), `privWrite` (stage in tmpdir → `sudo install -d -m 755` + `sudo install -m 644`), `privRemove` (`sudo rm -f --`), drop-in backups included, rollback async + sudo-aware; `Caddyfile`/`.env` keep the plain fs layer so ownership/mode is unchanged; `backend/SECURITY.md` documents the privilege model. **PR #60** (found by testing the deploy): `/etc/caddy` is root-owned too — the Caddyfile content, its `.bak` and its rollback restore now use `privWrite` as well (`EACCES: … open '/etc/caddy/Caddyfile.bak.<ts>'`). Tests emulate `install`/`rm` from the mocked exec and assert the exact privileged argv.
3. **The button that triggers it was disabled.** Look Up's `launch ▸` was disabled whenever `streams[char]` was missing — i.e. for exactly the chars that need provisioning — so auto-provisioning was unreachable from the UI (PR #59 enables it for `lich.write`/`characters.write`; read-only users keep a disabled button with an explanatory tooltip).

Verified live: `POST /api/modules/gameview/launch/Aeton` → 200; `gs4sd-lich@Aeton.service.d/override.conf` adds `--detachable-client=9105` (keeps `--start-scripts=gs4sd_publisher,accept,log,gs4sd_premium`); `vellum-fe@Aeton` (web 9205) enabled + active; Caddy `aeton.phylactery.ovh` + `reverse_proxy 127.0.0.1:9205`; `.env` `VELLUM_STREAMS=…,Aeton:9105:9205`; `https://aeton.phylactery.ovh/play` → 200 and the four pre-existing streams (fisternar/neleourg/vaikar/diynasta) still 200.

**Restart prompt (paste into a new session):**
> GSIVPlatform — repo `D:\Code Projects\GSIVPlatform`, server `ubuntu@51.68.235.144` (SSH key `~/.ssh/id_ed25519`), read `docs/STATUS.md` §7 first, then `AGENTS.md`. State as of 2026-09-10: Aeton (KAISER999, GS3 **live**) is up with a working VellumFE stream at `https://aeton.phylactery.ovh/play` (Lich detach 9105, web 9205); `/api/modules/gameview/launch/:char` now really auto-provisions streams for new characters (PRs #59/#60: privileged host mutations go through sudo) and the Look Up `launch ▸` button works for unprovisioned chars; the dead `JEMLEY/Aeton` entry.yaml record is gone. Gates: `cd backend && npm test && npm run typecheck && npm run lint` (383 tests, 0 lint errors) and `cd frontend && npm run build`; ship via branch + PR (`gh pr merge <n> --merge`), then deploy per `deploy/V2-DEPLOYMENT.md` (copy `dist` as a folder; frontend contents go to `/opt/gsiv-platform/frontend`, not `frontend/dist`) and restart `gsiv-platform.service`. Next candidates: (a) exercise the now-working auto-provision once on a brand-new character name (Fisternar/Neleourg only for testing, never Amn); (b) nothing else outstanding from the Aeton incident — the `ebounty_tracker.lic` → `/api/bounty/*` port is **parked by user decision (2026-09-10)**; do not start it unless the user asks. (c) **SECURITY FIRST:** the 2026-09-10 review (`docs/security/2026-09-10-security-review.md`) found the live admin/machine `AUTH_TOKENS` published in this **public** repo (the repo also publishes the origin IP) and Cloudflare running `Flexible` SSL with no TLS at the origin — the documented Phase 0 (rotate both tokens, update every consumer, purge them from docs) is unstarted; do it before any other feature work.
...
- Streams: VellumFE headless per char — Lich `--detachable-client` (Fisternar 9101, Neleourg 9102, Vaikar 9103), `vellum-fe@<Char>` units (web 9201/9202/9203), Caddy `<char>.phylactery.ovh`; URL `#token=<t>&lich=127.0.0.1:<port>&name=<char>`; no basic_auth. app.js carries two GSIVPlatform patches (zero-click auto-connect; form-flash suppression, 2026-08-12). Rebuild recipe: `deploy/V2-DEPLOYMENT.md` §VellumFE.
...
**Remaining (in order):**
1. ~~**Stream more chars** when they come online (3-step recipe)~~ — **AUTOMATED 2026-08-17** (PR #56) and **proven live 2026-09-10** (PRs #59/#60 fixed the EACCES/privilege bug that had kept it dead in production — see the session log below). Starting a new character now really does bring its stream up automatically.
2. (Optional) exercise the auto-provisioner once on a brand-new character name (use Fisternar/Neleourg only; never Amn).
3. **SKIPPED — user decision 2026-09-10:** port `ebounty_tracker.lic` (`/api/bounty/*`) into v2. Parked on purpose; do not pick it up unless the user asks again.
4. **SECURITY — findings documented 2026-09-10 (not fixed):** see `docs/security/2026-09-10-security-review.md`. 2 critical (the live admin + machine `AUTH_TOKENS` are committed in this **public** repo; Cloudflare is in `Flexible` SSL with no TLS listener at the origin), 4 high, 5 medium/low. The staged action plan (Phases 0–4) is in §6 of that document; **Phase 0 — rotating both tokens and purging them from `docs/**` — is unstarted and needs the user's go-ahead.**
...
**Session log — 2026-08-17 (Vaikar stream provisioned, server-only — no code changes):** user started the char **Vaikar** (Lich session live, `gs4sd-lich@Vaikar` running) but there was no VellumFE stream, so `launch ▸`/Watch was disabled. Fixed via the §VellumFE recipe: added `gs4sd-lich@Vaikar.service.d/override.conf` (`--detachable-client=9103`, kept Vaikar's start-scripts `gs4sd_publisher,accept,log,gs4sd_premium`) + restarted the unit; created + enabled `vellum-fe@Vaikar` (ports 9103/9203, `--now`); Caddyfile gained `vaikar.phylactery.ovh` → 127.0.0.1:9203 (backup + validate + reload); `VELLUM_STREAMS` in server .env → `...,Vaikar:9103:9203` (backup + restart gsiv-platform). Verified: streams API shows `Vaikar up:true`, `https://vaikar.phylactery.ovh/play` → 200, detach 9103 + web 9203 listening, vellum-fe journal clean. **Not enabled:** `gs4sd-lich@Vaikar` stays disabled (watchdog gating rule) — if Vaikar should auto-start on reboot, that is a separate decision.

**Session log — 2026-08-13 (late resume + wrap-up — no code changes):** resumed from the restart prompt below; verified the repo is clean and fully synced with origin/main at `9f2a600` (PR #53 — docs-only: deploy runbook node_modules-ownership gotcha + server runbook sync). Re-confirmed the live state: scan orchestrator (/scans, PR #48), lookup steps 1–6, roster-sync + play.net inactive scrape (PR #51), no-active-chars alert (PR #50), char-failure disambiguation (PR #49) — all deployed + smoke-tested on Fisternar/Neleourg. No new work taken on (user's direction: wrap up + refresh the restart prompt). Open items unchanged: (1) stale-char cleanup — the "Clean up stale" button is built + deployed (PR #47, TOTP-gated, `dry_run` preview) and only awaits the user's click on /accounts with a fresh code (17 dead accts + 30 stale chars); (2) optional ebounty_tracker port; (3) stream more chars when they come online (§VellumFE recipe).
...
> **ALL SHIPPED FEATURES LIVE (through PR #53, main @ 9f2a600):** scan orchestrator — new /scans page +
