...
- **Auto-provision (2026-08-17):** `POST /launch/:char` for a char not in `VELLUM_STREAMS` now provisions the stream automatically (Lich `--detachable-client`, `vellum-fe@` unit, Caddy host, `.env` entry) via the review-gated `core/stream-provision.ts` — no manual Caddy/.env edits. Next-free 910X/920X ports.
- **Caddy gotcha (hit 2026-08-17, manual edit):** each per-char `handle @v… { reverse_proxy 127.0.0.1:<web> }` must be a **top-level sibling** of the others (one tab indent). A hand-edit that nested `@vaikar` inside the `@vneleourg` block still passed `caddy validate` but made `vaikar.phylactery.ovh/play` serve an **empty body** (blank stream page) while locally the web port worked. After any Caddy change for a char, verify the PUBLIC `https://<char>.phylactery.ovh/play` returns the full HTML (curl size ≈ 19471 for the app shell), not size 0.
- **Rebuild / upgrade recipe → `deploy/VELLUMFE-UPGRADE.md`** (the playbook: detect → port the patch → build → swap → verify → rollback → record), with its two artifacts: `deploy/vellumfe/gsiv-webui.patch` (our exact web-UI delta — zero-click `autoConnectLich` + no-flash `zeroClickConnecting`) and `deploy/vellumfe/build-vellumfe.sh` (box-side build from a public upstream tag; prints one greppable `BUILD-OK … sha256=…` line). **History:** the running beta.37 binary was hand-built on the box on 2026-08-11 (clone `/opt/vellumfe-src`, edit `app.js`, `cargo build --release`, stop units → back up binary → swap → start). That hand delta is now captured as the patch artifact and verified byte-identical (sha256 `6f2a408f…`). Stream URLs (gameview module) must use `#lich=<host>:<port>&name=<char>` (NOT `rhost/rport` — that prefills the Remote tab, not the Lich tab).
